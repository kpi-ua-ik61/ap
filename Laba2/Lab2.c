...

{
	...
	if(scanf("%f%c", &side_A, &term) != 2 && term !="\n") //Проверка, точнее пропускной пункт
	{
		wrong();
	} 
	printf("\n Recieved ");
	...
	if(scanf("%f%c", &side_B, &term) != 2 && term !="\n") // Тут тоже пропускной пункт
	{
		wrong();
	}
	printf("\n Recieved ");
	...
	if(scanf("%f%c", &side_C, &term) !=2 && term !="\n") // И это пропускной пункт
	{
		wrong();
	}
	printf("\n Recieved ");
...
void wrong() // Сообщение о неправильности
{
	printf("\n Wrong Data! \n");  
	return;
}
